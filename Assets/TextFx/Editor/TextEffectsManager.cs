...
//			}
					...
					if(action.m_action_type == ACTION_TYPE.ANIM_SEQUENCE)
					{
						...
						DrawActionVariableSection(new GUIContent("Position", "Define a start/end positional transition for the text during this Action."),
							...
							if(!action.m_offset_from_last)
							{
								...
								CheckGUIChange(action_idx, true, ANIMATION_DATA_TYPE.POSITION);
							}
		...
		{	
			if(m_animation_manager == null || m_animation_manager.NumAnimations == 0 || m_animation_manager.Transform == null)
				return;
			...
			int action_idx = 0;
			Vector3 position_offset = m_animation_manager.Transform.position;
			
			LetterAnimation letter_animation;
			for(int anim_idx=0; anim_idx < m_animation_manager.NumAnimations; anim_idx++)
			{
				letter_animation = m_animation_manager.GetAnimation(anim_idx);
				action_idx= 0;
				...
				for(int idx=0; idx < letter_animation.NumActions; idx++)
				{
					...
					{
						action.m_start_pos.BezierCurve.OnSceneGUI(position_offset, m_animation_manager.Scale);
					...
					{
						action.m_end_pos.BezierCurve.OnSceneGUI(position_offset, m_animation_manager.Scale);
			...
			if(edit_detected)
			{
				m_animation_manager.SetAnimationState(edited_action, start_end_state, true);
