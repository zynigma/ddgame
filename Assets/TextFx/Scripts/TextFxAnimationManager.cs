...
//#if UNITY_EDITOR
		...
		public void SetQuickSetupSection(PRESET_ANIMATION_SECTION section, string animationName = "None", bool setEditorSectionIndex = false)
		{
			...
			if(animationName != "None")
			{
				...
				if(effectJsonDataString != "")
				{
					...
					ImportData(effectJsonDataString, preset_anim_section, section, true);
...
//#endif
	...
	//			Debug.Log("char '" + current_char + "' , visible : " + visible_character);
				...
				if(last_quad_visible && current_char.Equals('\n'))
					line_idx++;
...
//			Debug.Log ("PrepareAnimationData () " + what_to_update);
			...
			if(m_master_animations != null)
			{
				...
				// Render any un-animated letters in their default states
		...
		public bool UpdateMesh(bool use_timer, bool force_render, int action_idx = 0, float action_progress = 0, float delta_time = 0)
		{
			...
			if(m_master_animations != null)
			{
				...
				foreach(LetterAnimation animation in m_master_animations)
				{
					...
					if(animation.m_letters_to_animate == null)
						animation.m_letters_to_animate = new List<int>();
					...
					foreach(int letter_idx in animation.m_letters_to_animate)
					{
						...
						if(use_timer)
						{
							bool vertsChanged = letter_setup.AnimateMesh(this,
												                         force_render,
												                         m_animation_timer,
							...
							if(!vertsChanged)
							{
								// Use last animated colour setup
								letter_colours = letter_setup.CurrentAnimatedCols;
							}
