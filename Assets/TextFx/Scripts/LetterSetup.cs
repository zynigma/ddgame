...
namespace TextFx
{
	...
	public class LetterSetup
	{
		...
		[SerializeField]
		Vector3 m_anchor_offset_upper_left,
				m_anchor_offset_upper_center,
				m_anchor_offset_upper_right,
				m_anchor_offset_middle_left,
				m_anchor_offset_middle_center,
				m_anchor_offset_middle_right,
				m_anchor_offset_lower_left,
				m_anchor_offset_lower_center,
				m_anchor_offset_lower_right;

		
		// Getter / Setters
		public Vector3[] BaseVertices { get { return m_base_vertices; } }
		public Vector3 BaseVerticesTL { get { return m_base_vertices [m_base_indexes [0]]; } }
		public Vector3 BaseVerticesTR { get { return m_base_vertices [m_base_indexes [1]]; } }
		public Vector3 BaseVerticesBR { get { return m_base_vertices [m_base_indexes [2]]; } }
		public Vector3 BaseVerticesBL { get { return m_base_vertices [m_base_indexes [3]]; } }
        ...
        public Vector3[] BaseExtraVertices { get { return m_base_extra_vertices; } }
		...
		public Vector3 CenterLocal { get { 
				if(m_current_animated_vertices != null)
					...
					return m_anchor_offset_middle_center;
			} }
		...
		public void SetLetterData(Vector3[] mesh_verts, Color[] mesh_cols, Vector3[] extra_mesh_verts, Color[] extra_mesh_cols, int mesh_index)
		{
			...
			PreCalculateMeshInformation ();

            // Setup base colours array with correctly mapped colour values for this mesh
            m_base_colours = mesh_cols != null && mesh_cols.Length == 4 ? new VertexColour(mesh_cols[m_base_indexes[0]], mesh_cols[m_base_indexes[1]], mesh_cols[m_base_indexes[2]], mesh_cols[m_base_indexes[3]]) : new VertexColour(Color.white);

            // Do the same for any extra colour values
            if (extra_mesh_cols != null)
            {
                m_base_extra_colours = new Color[extra_mesh_cols.Length];

                for (int midx = 0; midx < extra_mesh_cols.Length / 4; midx ++)
                {
                    for (int vidx = 0; vidx < 4; vidx++)
                        m_base_extra_colours[(midx * 4) + vidx] = extra_mesh_cols[(midx * 4) + m_base_indexes[vidx]];
                }
            }
            else
                m_base_extra_colours = null;
        }

		void PreCalculateMeshInformation()
		{
			...
			m_anchor_offset_middle_center = ( m_base_vertices [0] + m_base_vertices [1] + m_base_vertices [2] + m_base_vertices [3] ) / 4f;

			m_anchor_offset_upper_left = m_anchor_offset_middle_center + new Vector3 (-half_width, half_height, 0);
			m_anchor_offset_upper_center = m_anchor_offset_middle_center + new Vector3 (0, half_height, 0);
			m_anchor_offset_upper_right = m_anchor_offset_middle_center + new Vector3 (half_width, half_height, 0);
			m_anchor_offset_middle_left = m_anchor_offset_middle_center + new Vector3 (-half_width, 0, 0);
			m_anchor_offset_middle_right = m_anchor_offset_middle_center + new Vector3 (half_width, 0, 0);
			m_anchor_offset_lower_left = m_anchor_offset_middle_center + new Vector3 (-half_width, -half_height, 0);
			m_anchor_offset_lower_center = m_anchor_offset_middle_center + new Vector3 (0, -half_height, 0);
			m_anchor_offset_lower_right = m_anchor_offset_middle_center + new Vector3 (half_width, -half_height, 0);
		}
		...
		Vector3 GetAnchorOffset(TextfxTextAnchor letter_anchor )
		{
			switch (letter_anchor)
			{
				case TextfxTextAnchor.UpperLeft:
					return m_anchor_offset_upper_left;
				case TextfxTextAnchor.UpperCenter:
					return m_anchor_offset_upper_center;
				case TextfxTextAnchor.UpperRight:
					return m_anchor_offset_upper_right;
				case TextfxTextAnchor.MiddleLeft:
					return m_anchor_offset_middle_left;
				case TextfxTextAnchor.MiddleCenter:
					return m_anchor_offset_middle_center;
				case TextfxTextAnchor.MiddleRight:
					return m_anchor_offset_middle_right;
				case TextfxTextAnchor.LowerLeft:
					return m_anchor_offset_lower_left;
				case TextfxTextAnchor.LowerCenter:
					return m_anchor_offset_lower_center;
				case TextfxTextAnchor.LowerRight:
					return m_anchor_offset_lower_right;
				default:
					return m_anchor_offset_middle_center;
			}
		}
		...
		public void SetMeshState(TextFxAnimationManager animation_manager_ref, int action_idx, float action_progress, LetterAnimation animation, AnimatePerOptions animate_per, ref Vector3[] mesh_verts, ref Color[] mesh_colours)
		{
			...
			else
			{
                ...
                {
                    m_base_extra_vertices.CopyTo(mesh_verts, 0);
                    offsetIndex = m_base_extra_vertices.Length;
                }
				m_base_vertices.CopyTo(mesh_verts, offsetIndex);
...
//			Debug.Log ("CallContinueCallback " + m_mesh_index);
                                  	...
                                  	ref Color[] mesh_colours)
		{
			...
			if(animation.NumActions > 0 && m_anim_state_vars.m_action_index < animation.NumActions && ( m_anim_state_vars.m_active || force_render ))
			{
				...
				float action_delay = m_ignore_action_delay || m_anim_state_vars.m_reverse ? 0 : m_action_delay;
				bool altered_delay = false;
				...
				{
					float old_action_delay = m_action_delay;
				   ...
				   || m_animation_manager_ref.WhatJustChanged == ANIMATION_DATA_TYPE.DURATION))
				{
					...
					m_anim_state_vars.m_timer_offset = -1 * (((m_anim_state_vars.m_reverse ? 1 - m_anim_state_vars.m_linear_progress : m_anim_state_vars.m_linear_progress) * m_action_duration) - timer + (m_anim_state_vars.m_reverse ? 0 : action_delay));
				...
				if((m_anim_state_vars.m_reverse || m_action_timer > action_delay))
				{
					m_anim_state_vars.m_linear_progress = (m_action_timer - (m_anim_state_vars.m_reverse ? 0 : action_delay)) / m_action_duration;
					...
					if(m_visible_character && !m_anim_state_vars.m_started_action && m_current_letter_action != null)
					{
						...
						TriggerEffects<ParticleEffectSetup>(
										m_current_letter_action.ParticleEffectSetups,
						               	animate_per,
						               	PLAY_ITEM_EVENTS.ON_START,
										(ParticleEffectSetup effect_setup) => { 
					...
					if((!m_anim_state_vars.m_reverse && m_anim_state_vars.m_linear_progress >= 1) || (m_anim_state_vars.m_reverse && m_action_timer >= m_action_duration + action_delay))
					{
...
//							Debug.Log("[" + m_mesh_index + "] current action index : "+  m_anim_state_vars.m_action_index + ", triggerIndex : "+ m_continueActionIndexTrigger + ", [" + ActiveLoopCycles[0].m_active_loop_index + "] loop_num_left : "+ ActiveLoopCycles[0].m_number_of_loops);
			...
			{
				// no actions found for this letter. Position letter in its last animated state, or else its default position
				mesh_verts = m_current_animated_vertices != null ? m_current_animated_vertices : BaseVertices;

				mesh_colours = m_current_animated_colours != null ? m_current_animated_colours : new Color[]{Color.white, Color.white, Color.white, Color.white};
		...
		void TriggerEffects<T>(List<T> effectSetups, AnimatePerOptions animate_per, PLAY_ITEM_EVENTS play_when, System.Action<T> play_effect_callback) where T : EffectItemSetup
		{
				...
				foreach(T effect_setup in effectSetups)
				{
					   ...
					   && (effect_setup.m_effect_assignment == PLAY_ITEM_ASSIGNMENT.PER_LETTER || effect_setup.m_effect_assignment_custom_letters.Contains(m_progression_variables.LetterValue)))
					{
						if(	!effect_setup.m_loop_play_once ||
							...
							m_anim_state_vars.ActiveLoopCycles[0].FirstPass)
						{
							play_effect_callback(effect_setup);
						}
					}
				}
			}
		...
		void CacheCurrentStateToAction(LetterAction end_action, LetterAnimation letterAnimation, bool use_start_state, AnimatePerOptions animate_per)
		{
			...
			m_colour_to = use_start_state ? end_action.m_start_colour.GetValue(all_letter_actions, m_progression_variables, animate_per, letterAnimation.m_defaultTextColourProgression) : end_action.m_end_colour.GetValue(all_letter_actions, m_progression_variables, animate_per, letterAnimation.m_defaultTextColourProgression);
		...
		void CacheLetterActionStartEndStates(LetterAction current_action, LetterAnimation letterAnimation, AnimatePerOptions animate_per)
		{
			...
			m_local_scale_from = current_action.m_start_scale.GetValue(all_letter_actions, m_progression_variables, animate_per, true);
			m_local_scale_to = current_action.m_end_scale.GetValue(all_letter_actions, m_progression_variables, animate_per, true);

			// Calculate Global Scale Vector
			m_global_scale_from = current_action.m_global_start_scale.GetValue(all_letter_actions, m_progression_variables, animate_per, true);
			m_global_scale_to = current_action.m_global_end_scale.GetValue(all_letter_actions, m_progression_variables, animate_per, true);

			// Calculate Local Rotation
			m_local_rotation_from = current_action.m_start_euler_rotation.GetValue(all_letter_actions, m_progression_variables, animate_per, true);
			m_local_rotation_to = current_action.m_end_euler_rotation.GetValue(all_letter_actions, m_progression_variables, animate_per, true);

			// Calculate Global Rotation
			m_global_rotation_from = current_action.m_global_start_euler_rotation.GetValue(all_letter_actions, m_progression_variables, animate_per, true);
			m_global_rotation_to = current_action.m_global_end_euler_rotation.GetValue(all_letter_actions, m_progression_variables, animate_per, true);

			// Calculate Position
			m_position_from = current_action.m_start_pos.GetValue(all_letter_actions, m_progression_variables, animate_per, true);
			m_position_to = current_action.m_end_pos.GetValue(all_letter_actions, m_progression_variables, animate_per, true);

			// Sort out letters colour
			m_colour_from = current_action.m_start_colour.GetValue(all_letter_actions, m_progression_variables, animate_per, letterAnimation.m_defaultTextColourProgression);
			m_colour_to = current_action.m_end_colour.GetValue(all_letter_actions, m_progression_variables, animate_per, letterAnimation.m_defaultTextColourProgression);
		...
		void SetupMesh(LetterAction letter_action, float action_progress, float linear_progress, ref Vector3[] mesh_verts, ref Color[] mesh_colours)
		{
			...
			for(int idx=0; idx < mesh_verts.Length; idx++)
			{
				...
				mesh_verts[idx] = extraVertIdx >= 0 ? m_base_extra_vertices[extraVertIdx] : m_base_vertices[baseVertIdx];
				...
				mesh_verts[idx] += m_letter_position * m_animation_manager_ref.AnimationInterface.MovementScale;
