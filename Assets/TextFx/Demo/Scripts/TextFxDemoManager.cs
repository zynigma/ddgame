...
public class TextFxDemoManager : MonoBehaviour {
...
#if !UNITY_4_6 && !UNITY_4_7 && !UNITY_5_0 && !UNITY_5_1 && !UNITY_5_3
	...
	void Start ()
	{
		...
		int introAnimIndex = m_introStartEffect < 1 ? Random.Range(1,configAnimNames[0].Length) : Mathf.Clamp(m_introStartEffect, 1, configAnimNames[0].Length);
		int mainAnimIndex = m_mainStartEffect < 1 ? Random.Range(1,configAnimNames[1].Length) : Mathf.Clamp(m_mainStartEffect, 1, configAnimNames[1].Length);
		int outroAnimIndex = m_outroStartEffect < 1 ? Random.Range(1,configAnimNames[2].Length) : Mathf.Clamp(m_outroStartEffect, 1, configAnimNames[2].Length);
		...
		// Play intro sequence
		StartCoroutine(DoIntro(5, m_skipTitleEffectIntro));
    }
	...
	IEnumerator DoIntro(float waitTime, bool skipTitleEffect = false)
	{
		...
		m_titleEffect.AnimationManager.PlayAnimation (0.5f);
	...
	void AnimationSectionSet(TextFxAnimationManager.PRESET_ANIMATION_SECTION section, int animIndex, bool restartAnimation = false)
	{
		...
		m_effect.AnimationManager.SetQuickSetupSection(section, m_animSectionDropdowns[(int) section].options[animIndex].text);
